...
/**
 * sticker 滚动条
 * TODO: 切换 767px/768px 未重新计算周期
 */
$(document).ready(function () {
    var box0 = $(".one"), v0 = 0.9; //这里添加滚动的对象和其速率
    var box1 = $(".two"), v1 = 1;
    Rin(box0, v0);
    Rin(box1, v1);
...
// 动态加载友链
async function loadDynamicFriends() {
    const refreshBtn = document.querySelector('.flink-refresh#flink-r1');
    const friendList = document.querySelector('.flink-random'); // 友链容器
    let lang = window.location.pathname.includes('ja-JP') ? 1 : 0;
    ...
    try {
        ...
        const pool = friends.filter(friend => friend.state === 1 && friend.name !== "雨雪冰屋");
        ...
        // 生成动态友链HTML
        const dynamicHTML = selected.map(f => `
            <li>
                <a href="${f.url}" class="link-footer" target="_blank" lang="zh-CN">
                    <img 
                        class="lazy" 
                        src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=" 
                        data-src="${f.logo}" 
                        alt="${f.name}" 
                        style="width:18px;margin-right:2px;margin-bottom:3px;"
                    >
                    ${f.name}
                </a>
            </li>
        ...
        friendList.innerHTML = `
            <li>Something went wrong. Please refresh later.</li>
            <li>
                <a href="/ask/friends" class="link-footer set-lang">
                    <small class="fa fa-ellipsis-h" style="min-width:17px;size:15px;margin-left:3px;"></small>
                    `+ (lang == 0 ? `更多友链`:`もっと`) +`
                </a>
            </li>
